...
<details>
<summary>点击展开查看隐藏的惊喜</summary>

这是一个原生的 HTML 折叠块，常用于隐藏长代码或剧透内容。
结合 **Markdown** 语法，内部依然可以正常渲染：
...
这是 <u>带有下划线的文本</u>，这是 <ins>插入的文本</ins>。

Hexo
: 快速、简洁且高效的博客框架

Butterfly
: 一个基于 Hexo 的高颜值且功能强大的主题
...
\begin{Bmatrix} x & y \\ z & w \end{Bmatrix} \quad
...
分子：$\ce{H2O}$, $\ce{Sb2O3}$, $\ce{AgCl2-}$
离子：$\ce{H+}$, $\ce{CrO4^2-}$, $\ce{[AgCl2]-}$
同位素：$\ce{^{227}_{90}Th+}$, $\ce{^227_90Th+}$
复杂结构：$\ce{KCr(SO4)2 * 12H2O}$
...
$$
\ce{A <--> B} \quad \text{(等长箭头)}
$$

$$
\ce{A <=>> B} \quad \text{(平衡正向移动)}
$$
...
$$
\ce{CH3-CH2-OH} \quad \text{或} \quad \ce{CH3\bond{=}CH2} \quad \text{或} \quad \ce{A\bond{~}B\bond{~-}C}
$$

核反应方程式：

$$
\ce{^{235}_{92}U + ^{1}_{0}n -> ^{137}_{56}Ba + ^{97}_{36}Kr + 2^{1}_{0}n}
$$
